import { useLoaderData, useParams, useSearchParams, type LoaderFunction } from "react-router"
...
const PageSearch = () => {
	...
	const seachOption = params.searchOption // 用于判断是issue还是book
	...
	const issueList =
		results.length > 0 ? (
			results.map((it: IssueInfo | BookInfo, index: number) => (
				<SearchResultItem info={it} key={index}></SearchResultItem>
			))
		) : (
...
export const loader: LoaderFunction = async ({ params, request }) => {
	...
	// 根据searchOption判断是问答还是书籍
	let res
	if (searchOption === "issue") {
		res = await issueApi.getIssueByTitleAndContent({
			page,
			page_size,
			searchValue,
			searchOption,
		...
		res = await bookApi.getBookByTitleAndContent({
			page,
			page_size,
			searchValue,
			searchOption,
